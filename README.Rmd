...
surface with the optional `rivers` and `waterbodies` arguments. River edges are
directed — faster downstream than upstream — while standing water is isotropic.
Flow direction is derived robustly from the river geometry and the overall
elevation trend, so noise in the DEM between adjacent cells does not flip it.
...
The resulting `cs_water` is used exactly like any other cost surface, e.g. in
`perform_tpla()` or `compute_lcp()`.
